    ...
    const loadData = async () => {
      try {
        ...
        if (branchData.length > 0) {
          const defaultBranchId = String(branchData[0].id);
          setStaffBranchId((currentValue) => currentValue || defaultBranchId);
          setInventoryBranchId(
            (currentValue) => currentValue || defaultBranchId,
          );
        }
      } catch {
        setNotice({
  ...
  return (
    <Stack spacing={3}>
      ...
      >
        ...
        >
          <Stack component="form" spacing={2} onSubmit={handleBranchSubmit}>
            <TextField
              label="Branch Name"
              value={branchName}
        ...
        >
          <Stack component="form" spacing={2} onSubmit={handleMenuSubmit}>
            <TextField
              label="Item Name"
              value={menuName}
        ...
        >
          <Stack component="form" spacing={2} onSubmit={handleInventorySubmit}>
            <SelectField
              label="Branch"
              ...
              <TableBody>
                {inventory.map((entry) => (
                  <TableRow key={`${entry.branch_id}-${entry.item_id}`}>
                    <TableCell>
                      {menuItems.find((item) => item.id === entry.item_id)
                        ?.name ?? entry.item_id}
                    </TableCell>
